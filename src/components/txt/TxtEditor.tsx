...
const CANVAS_STORAGE_KEY = 'txt-diagram-canvas-state'
...
function saveEditorState(state: EditorPanelState) {
  try { localStorage.setItem(EDITOR_STORAGE_KEY, JSON.stringify(state)) } catch { /* ignore */ }
}
...
function saveCanvasState(diagramId: string, state: CanvasState) {
  try { localStorage.setItem(`${CANVAS_STORAGE_KEY}-${diagramId}`, JSON.stringify(state)) } catch { /* ignore */ }
}
...
export function TxtEditor({
  ...
  // dark mode 监听
  useEffect(() => {
    const update = () => setIsDarkMode(window.document.documentElement.classList.contains('dark'))
    update()
    const obs = new MutationObserver(update)
    obs.observe(window.document.documentElement, { attributes: true, attributeFilter: ['class'] })
  ...
  // 切换图表时重置 canvas 状态
  useEffect(() => {
    setCanvasState(loadCanvasState(diagramId))
  }, [diagramId])

  useEffect(() => { saveEditorState(panelState) }, [panelState])

  // 保存
  const handleSave = useCallback(async (isAuto = false) => {
    if (!currentDiagram || currentDiagram.type !== 'txt') return
    await createSnapshot(currentDiagram.id, currentDiagram.source, isAuto ? '自动保存' : '手动保存', isAuto)
  ...
  useEffect(() => {
    ...
    if (hasChanges && currentDiagram && settings.autoSaveInterval > 0) {
      autoSaveTimerRef.current = window.setTimeout(() => handleSave(true), settings.autoSaveInterval)
    }
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      lastMousePosRef.current = { x: e.clientX, y: e.clientY }
      setCanvasState((prev) => {
        const updated = { ...prev, offsetX: prev.offsetX + dx, offsetY: prev.offsetY + dy }
        saveCanvasState(diagramId, updated)
        return updated
      })
    }
  ...
  const handleExport = useCallback(async () => {
    ...
    await exportDiagram({ ...currentDiagram, source })
  }, [currentDiagram, source])

  if (!currentDiagram || currentDiagram.type !== 'txt') {
    return (
  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      ...
      >
        ...
        >
          <pre
            className="text-sm leading-relaxed whitespace-pre-wrap break-words font-mono p-8 max-w-3xl"
            style={{ color: 'var(--foreground)' }}
          >
            {source || <span className="text-muted-foreground">（空文档）</span>}
          </pre>
        </div>
      ...
      >
        ...
        <div className="flex items-center gap-2 p-2 border-b shrink-0">
          <div className="flex items-center gap-1 ml-auto">
            <Button variant="outline" size="sm" onClick={() => handleSave(false)} disabled={!hasChanges} className="h-8 text-xs">
        ...
        >
          {activeTab === 'code' ? (
            <CodeEditor
              value={source}
              onChange={(val) => { setSource(val); setHasChanges(true) }}
                ...
                {snapshots.length === 0 ? (
                  ...
                  snapshots.map((snapshot) => (
                    <div key={snapshot.id} className="flex items-center justify-between p-2 border rounded-lg text-sm">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <Button variant="outline" size="sm" onClick={() => restoreSnapshot(snapshot.id)} className="h-7 text-xs">
                          恢复
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => deleteSnapshot(snapshot.id)} className="h-7 text-xs">
                          删除
                        </Button>
                      </div>
                    </div>
